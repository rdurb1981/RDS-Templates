...
if ($LogAnalyticsWorkspaceId -and $LogAnalyticsPrimaryKey)
{
	...
	function PeakHours-StartSessionHosts-DF
	{
		...
		foreach ($SessionHost in $AllSessionHosts) {
			...
			# Check the Session host is in maintenance
			Check-IfSessionHostInMaintenance -VMName $VMName -MaintenanceTagName $MaintenanceTagName -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
					...
					if ($SessionHost.Status -ne "NoHeartbeat") {
						if ($NumberOfRunningHost -gt $MinimumNumberOfRDSH) {
							...
							else
							{
								...
								$UserSessionsOfHost = SignOfUserSessions -TenantName $TenantName -HostPoolName $HostPoolName -SessionHost $SessionHostName -LimitSecondsToForceLogOffUser $LimitSecondsToForceLogOffUser -LogOffMessageTitle $LogOffMessageTitle -LogOffMessageBody $LogOffMessageBody -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
								# Check the Session host is in maintenance
								Check-IfSessionHostInMaintenance -VMName $VMName -MaintenanceTagName $MaintenanceTagName -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
	...
	$AllSessionHosts = Check-IfHostpoolHaveSessionHosts -TenantName $TenantName -HostPoolName $HostpoolName -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
	if ($HostpoolInfo.LoadBalancerType -eq "DepthFirst")
	{
		...
		if ($CurrentDateTime -ge $BeginPeakDateTime -and $CurrentDateTime -le $EndPeakDateTime)
		{
			...
			PeakHours-StartSessionHosts-DF -TenantName $TenantName -HostPoolName $HostpoolName -SessionhostLimit $SessionhostLimit -HostpoolMaxSessionLimit $HostpoolMaxSessionLimit -MinimumNumberOfRDSH $MinimumNumberOfRDSH -MaintenanceTagName $MaintenanceTagName -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
		}
		else {
			...
			$UsageresultShs = OffPeakUserSessionUsage-SpinUpSessionHost -NumberOfRunningHost $OffpeakhoursShs -MinimumNumberOfRDSH $MinimumNumberOfRDSH -DefinedMinimumNumberOfRDSH $DefinedMinimumNumberOfRDSH -TenantName $TenantName -HostPoolName $HostpoolName -Type "DepthFirst" -MaintenanceTagName $MaintenanceTagName -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
		...
		if ($CurrentDateTime -ge $BeginPeakDateTime -and $CurrentDateTime -le $EndPeakDateTime)
		{
			...
			PeakHours-StartSessionHosts-BF -TenantName $TenantName -HostPoolName $HostpoolName -MinimumNumberOfRDSH $MinimumNumberOfRDSH -SessionThresholdPerCPU $SessionThresholdPerCPU -MaintenanceTagName $MaintenanceTagName -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
		}
		else
		{
			Write-Output "It is Off-peak hours"
	...
	Helper functions
	#>
	# Function to check and update the loadbalancertype in peak and offpeak hours with "BreadthFirst"
	function UpdateLoadBalancerTypeInPeakandOffPeakwithBredthFirst{
	param(
		[string]$HostpoolLoadbalancerType,
		[string]$TenantName,
	...
	[int]$MaxSessionLimitValue = $HostpoolInfo.MaxSessionLimit
	if ($CurrentDateTime -ge $BeginPeakDateTime -and $CurrentDateTime -le $EndPeakDateTime) {
	UpdateLoadBalancerTypeInPeakandOffPeakwithBredthFirst -HostpoolLoadbalancerType $HostpoolLoadbalancerType -TenantName $TenantName -HostpoolName $HostpoolName -MaxSessionLimitValue $MaxSessionLimitValue
	}
	else {
	UpdateLoadBalancerTypeInPeakandOffPeakwithBredthFirst -HostpoolLoadbalancerType $HostpoolLoadbalancerType -TenantName $TenantName -HostpoolName $HostpoolName -MaxSessionLimitValue $MaxSessionLimitValue
	}
